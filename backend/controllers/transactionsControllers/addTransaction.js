...
const addTransaction = async (req, res, next) => {
  ...
  lineBreak();
  console.log(
    `НОВАЯ TransactionExpenses с ID: ${transaction.id}:`.bgYellow.blue
  ); //!
  ...
  const { balance } = user;
  console.log(
    `Старый Баланс пользователя с ID: ${userId} = ${balance} UAN `.bgBlue.red
  ); //!
  console.log();
  ...
  console.log(
    `Новый БАЛАНС пользователя с ID: ${userId} = ${balanceNew} UAN `.bgBlue.red
  ); //!
